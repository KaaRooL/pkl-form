...
export default function Form(){
  ...
  return( 
    <div>
      ...
      {isPopupVisible && <Popup onClose={()=> setPopupVisibility(false)}></Popup>}
  ...
  <div className={styles["form-group"]}>
    ...
    {errors.email?.type == "pattern" && <span>Email musi być zapisany w odpowiednim formacie.</span>}
    {errors.email?.type == "maxLength" && <span>Adres email nie może być dłuższy, niż 100 znaków.</span>}
  </div>
  <div className={styles["form-group"]}>
  <label className={styles.label} htmlFor="phone">Numer telefonu</label>
    ...
    {errors.phone?.type == "pattern" && <span>Numer telefonu powinien składać się tylko z cyfr.</span>}
    {errors.phone?.type == "maxLength" && <span>Numer telefonu nie może być dłuższy, niż 15 cyfr.</span>}
    
  </div>
  <button type="submit" disabled={isSubmitting} className={styles.btn}> Wyślij </button>
